...
export default function Index({ units, filters, branches, canPickBranch }) {
    ...
    const { data, setData, post, put, processing, errors, reset, clearErrors } = useForm({
        floor: '', block: '', unit_number: '', unit_code: '', size: '', branch_id: '', is_active: true,
    });

    useEffect(() => {
        if (editingUnit) return; // jangan auto-ubah kode saat edit
    ...
    const startEdit = (unit) => {
        setEditingUnit(unit);
        setData({
            floor: unit.floor, block: unit.block ?? '', unit_number: unit.unit_number,
            unit_code: unit.unit_code, size: unit.size ?? '', branch_id: unit.branch_id, is_active: unit.is_active,
        });
        clearErrors();
    };

    const cancelEdit = () => { setEditingUnit(null); reset(); clearErrors(); };

    const submit = (e) => {
        e.preventDefault();
        const options = { preserveScroll: true, onSuccess: () => { setEditingUnit(null); reset(); } };
        editingUnit ? put(`/units/${editingUnit.id}`, options) : post('/units', options);
    };

    const handleDelete = (id) => {
        if (confirm('Hapus unit ini?')) router.delete(`/units/${id}`, { preserveScroll: true });
    };

    const updateFilter = (value) => router.get('/units', { search: value }, { preserveState: true });
        ...
        <AppLayout>
            <div className="px-6 sm:px-8 py-6 flex-1">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 items-start">
                    <form onSubmit={submit} className="bg-white rounded-xl border border-[#E2E5EA] p-5 lg:sticky lg:top-6">
                        ...
                        <FormField label="Kode Unit" error={errors.unit_code} hint="Otomatis, bisa diedit manual.">
                            <TextInput value={data.unit_code} onChange={(e) => setData('unit_code', e.target.value)} />
                        </FormField>

                        <FormField label="Luas (m²)" error={errors.size}>
                            <NumberInput step="0.01" value={data.size} onChange={(e) => setData('size', e.target.value)} />
                        </FormField>

                        <Checkbox label="Unit aktif" checked={data.is_active} onChange={(e) => setData('is_active', e.target.checked)} className="mt-1 mb-4" />
                        ...
                        <DataTable columns={columns}>
                            {units.data.map((unit) => (
                                ...
                                >
                                    ...
                                    <td className="px-5 py-3.5 text-gray-500">{unit.active_tenancy?.tenant?.name ?? '—'}</td>
                                    <td className="px-5 py-3.5 text-right">
                                        <Badge color={unit.is_occupied ? 'blue' : 'gray'}>{unit.is_occupied ? 'Terisi' : 'Kosong'}</Badge>
                                    </td>
                            ...
                            {units.data.length === 0 && (
                                <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-gray-400">Belum ada unit.</td></tr>
                            )}
